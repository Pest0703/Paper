...
当前版本：`0.3.2` · 当前状态：Beta

0.3.2 新增论文目录永久移除功能。删除只影响 PaperTutor 本地数据，不触碰原始论文文件；目录记录、解析 Profile、OCR、书签、AI/OCR 缓存、Word 转换中间文件、论文笔记及笔记图片会在二次确认后删除且不保留备份。再次导入同一文件时按全新论文处理。
...
- 论文目录支持永久移除论文的软件数据，删除前明确列出范围且始终保留原始 PDF、DOC 或 DOCX 文件
...
- OCR 结果显示在可关闭的独立中间文本页，不再伪装成导师回答；未识别时显示“未进行OCR”，每次识别完成后自动打开
- OCR 文本页支持多页结果与原生文字选取，选中的内容进入右侧可编辑确认框后再向导师提问
