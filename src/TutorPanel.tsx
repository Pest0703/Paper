...
export type Turn={role:'user'|'assistant';content:string;call?:AiCallRecord};
type Capture={dataUrl:string;page:number;width?:number;height?:number};
type Props={selected:string;originalSelected:string;capture:Capture|null;answer:string;answerCall:AiCallRecord|null;turns:Turn[];busy:boolean;error:string;onAsk:(q:string)=>void;onAction:(q:string)=>void;onEditSelection:(text:string)=>void;onResetSelection:()=>void;onReanalyze:(question:string)=>void;onAnalyzeCapture:(question:string)=>void;onClearCapture:()=>void;onCancel:()=>void;onClose:()=>void;onJump:(p:number)=>void;promptDebug?:any};
...
export function TutorPanel({selected,originalSelected,capture,answer,answerCall,turns,busy,error,onAsk,onAction,onEditSelection,onResetSelection,onReanalyze,onAnalyzeCapture,onClearCapture,onCancel,onClose,onJump,promptDebug}:Props){
